	...
	public function process( array $page_ids ) {
		...
		if ( empty( $page_ids ) ) {
			add_settings_error(
				'notion_sync',
				'no_pages_selected',
				__( 'No pages selected. Please select pages to sync.', 'vger-sync-for-notion' ),
				'warning'
			);
			return;
		}
		...
		foreach ( $page_ids as $page_id ) {
			...
			try {
				...
				if ( $result['success'] ) {
					...
					++$error_count;
					$errors[] = $result['error'] ?? __( 'Unknown error', 'vger-sync-for-notion' );
				}
			} catch ( \Exception $e ) {
				++$error_count;
				$errors[] = $e->getMessage();
		...
		if ( 0 < $success_count && 0 === $error_count ) {
			...
			add_settings_error(
				...
				sprintf(
					...
					__( 'Synced %1$d pages. Failed to sync %2$d pages.%3$s', 'vger-sync-for-notion' ),
					$success_count,
					$error_count,
					$error_details
				),
			...
			add_settings_error(
				...
				sprintf(
					...
					__( 'Failed to sync %1$d pages.%2$s', 'vger-sync-for-notion' ),
					$error_count,
					$error_details
				),
				'error'
