...
/**
 * Converts Notion bulleted_list_item blocks to Gutenberg lists
 *
 ...
 * Adjacent list items should be grouped into a single <ul> element.
 *
 * @since 1.0.0
 */
class BulletedListConverter implements BlockConverterInterface {
	/**
	 ...
	 * @return bool True if block type is 'bulleted_list_item'.
	 */
	public function supports( array $notion_block ): bool {
		return isset( $notion_block['type'] ) && 'bulleted_list_item' === $notion_block['type'];
	}

	/**
	 * Convert Notion bulleted list item to Gutenberg list
	 *
	...
	public function convert( array $notion_block ): string {
		$rich_text = $notion_block['bulleted_list_item']['rich_text'] ?? array();
	...
	private function convert_children( array $children ): string {
		...
		foreach ( $children as $child_block ) {
			// Check if child is also a list item.
			if ( $this->supports( $child_block ) ) {
				$child_rich_text = $child_block['bulleted_list_item']['rich_text'] ?? array();
				...
				$html .= '<ul><li>' . $child_content . '</li></ul>';
			}
		}
