...
(function (wp) {
	...
	/**
	 * NotionLinkEdit Component - Edit interface for the Notion Link block
	 *
	 * @param {Object} props - Component props
	 * @return {Object} React element
	 */
	function NotionLinkEdit(props) {
		...
		// Render editor view
		return el('div', { className: 'wp-block-notion-sync-notion-link' }, [
			// Inspector Controls (sidebar)
			el(
				InspectorControls,
				{ key: 'inspector' },
				el(PanelBody, { title: __('Link Settings', 'notion-wp') }, [
					el(TextControl, {
						key: 'notion-id',
			...
			el(
				...
				{
					key: 'content',
					className: 'notion-link-block-editor',
				},
				[
					// Loading state
					loading &&
					...
					!loading &&
						...
						el(
							...
							el(
								...
								{},
								__(
									'Enter a Notion ID in the sidebar →',
									'notion-wp'
								)
							)
						),
							...
							[
								el(
									'a',
									{
										key: 'link',
										href: linkData.url,
										className: `notion-link notion-link--${
											linkData.type
										}`,
										target: openInNewTab
											? '_blank'
	...
	registerBlockType('notion-sync/notion-link', {
		...
		description: __(
			'Link to a Notion page or database with always-current title',
			'notion-wp'
		),
		icon: 'admin-links',
		category: 'embed',
